...
LDP.prototype.listContainer = function (filename, uri, containerData, callback) {
  ...
  function readdir(filename, callback) {
    ...
    fs.readdir(filename, function (err, files) {
      if (err) {
        ...
        return callback({ status:404, message: err });
      }
  ...
  function addFile(ldp, resourceGraph, baseUri, uri, container, file, callback) {
      ...
      // Get file stats
      fs.stat(container + file, function (err, stats) {
        if (err) {
        ...
        var fileBaseUri = utils.filenameToBaseUri(file, uri, ldp.root);
        ...
        getMetadataGraph(metaFile, fileBaseUri, function(err, metadataGraph) {
          if (err) {
  ...
  async.waterfall([
    ...
    function (next) {
      fs.stat(filename, function(err,  containerStats) {
        if (!err) {
          addStats(resourceGraph, baseUri, containerStats);
        }
        next();
      });
    },
