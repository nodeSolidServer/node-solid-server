...
class LDP {
  ...
  async put (url, stream, contentType) {
    ...
    // PUT without content type is forbidden. Except for container
    ...
    await this.createDirectory(path, hostname, !container) // , !container)
  ...
  async createDirectory (path, hostname, nonContainer = true) {
    try {
      ...
      if (!fs.existsSync(dirName)) {
        ...
        if (this.live && nonContainer) { // } && firstDirectoryRecursivelyCreated) {
