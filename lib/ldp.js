...
class LDP {
  ...
  async put (url, stream, contentType) {
    ...
    await this.checkNotFolderName(url)

    // check if file exists, and in that case that it has the same extension
    await this.checkFileExtension(url, path)
    ...
    try {
      ...
      if (this.live && firstDirectoryRecursivelyCreated) {
        ...
        const parentDirectoryPath =
          utilPath.dirname(dirName) + utilPath.sep
  ...
  async checkNotFolderName (url) {
    let container
    try {
      const folderUrl = (url.url || url) + '/'
      const { path: folderPath } = await this.resourceMapper.mapUrlToFile({ url: folderUrl })
      container = fs.lstatSync(folderPath).isDirectory()
    } catch (err) { container = false }
