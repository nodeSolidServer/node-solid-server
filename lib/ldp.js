...
class LDP {
  ...
  async deleteContainer (directory) {
    ...
    const { path: isRoot } = await this.resourceMapper.mapUrlToFile({ url: '/' })
    if (directory === isRoot) throw error(405, isRoot + 'DELETE of root is not allowed')
  ...
  async deleteDocument (filePath) {
    // DELETE Pod root acl, methods not allowed
    const { path: isRootAcl } = await this.resourceMapper.mapUrlToFile({ url: `/${this.suffixAcl}` }) // `/${this.suffixAcl}` })
    if (filePath === isRootAcl) throw error(405, isRootAcl + 'DELETE of root acl is not allowed')
