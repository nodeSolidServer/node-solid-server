...
LDP.prototype.post = function (hostname, containerPath, slug, stream, container, callback) {
  ...
  ldp.getAvailablePath(hostname, containerPath, slug, stream.get('Content-Type'), function (resourcePath) {
...
LDP.prototype.exists = function (host, reqPath, callback) {
  ...
  ldp.stat(filename, callback)
}

LDP.prototype.get = function (host, reqPath, baseUri, includeBody, acceptType, contentType, callback) {
  var ldp = this
  var filename = this.mapToFile(reqPath, host, acceptType)
  // var filename = utils.uriToFilename(reqPath, root)
  debug.handlers('GET DEBUG -- filename: ' + filename)
    ...
    if (stats.isDirectory()) {
      ...
      stream
        ...
        .on('open', function () {
          ...
          if (utils.hasSuffix(filename, ldp.turtleExtensions) || !contentType) {
...
LDP.prototype.delete = function (host, resourcePath, callback) {
  var ldp = this
  var filename = this.mapToFile(resourcePath, host, undefined)
  debug.handlers('DELETE -- Attempting to delete: ' + filename)
  ldp.stat(filename, function (err, stats) {
    if (err) {
      debug.handlers('DELETE -- Globbing because resource not found: ' + filename)
      // try to glob for $.ext files
      var globOptions = {
        noext: true,
        nobrace: true
      }
      ...
      glob(filename + '$*', globOptions, function (err, matches) {
        ...
        async.each(matches, function (match, done) {
          ldp.deleteResource(match, done)
        }, function (err) {
          debug.handlers('DELETE -- Done deleting globbed results')
          return callback(err)
        })
      })
    } else {
...
LDP.prototype.mapToFile = function (resource, host, contentType) {
  var ldp = this
  var root = !ldp.idp ? ldp.root : ldp.root + host + '/'
  var filename = utils.uriToFilename(resource, root)
  debug.handlers('GET DEBUG -- mapToFile path:' + filename)
  // Strip .
  var aclExt = ldp.suffixAcl.substr(1, ldp.suffixAcl.length)
  var metaExt = ldp.suffixMeta.substr(1, ldp.suffixMeta.length)

  if (S(filename).endsWith('/')) {
    return filename
  }

  debug.handlers('GET DEBUG -- mapToFile ctype:' + contentType)

  if (contentType && contentType !== '*/*') {
    var toExt = mime.extension(contentType)
    debug.handlers('GET DEBUG -- mapToFile toExt:' + toExt)
    // couldn't match content type to a known extension
    if (toExt.length === 0) {
      return filename
    }
    // attempt to guess current extension from URI
    var currExt = path.extname(filename)

    // return acl or meta filenames as they are
    if (currExt === aclExt || currExt === metaExt) {
      return filename
    }

    // guess content type from extension
    var fileCtype = mime.lookup(currExt)

    // no content type registered for this extension or ctype mismatch
    if (currExt.length === 0 || fileCtype !== contentType) {
      // map to $ext
      try {
        // check if we're able to access the file (exists)
...
LDP.prototype.getAvailablePath = function (host, containerURI, slug, contentType, callback) {
  var self = this
  var exists
  ...
  // Map slug to extension
  slug = path.basename(this.mapToFile(path.join(containerURI, slug), host, contentType))
  var newPath = path.join(containerURI, slug)

  // TODO: maybe a nicer code
  doWhilst(
    function (next) {
      var root = !self.idp ? self.root : self.root + host + '/'
      var filename = utils.uriToFilename(newPath, root)

      self.stat(filename, function (err, stats) {
        exists = !err
        if (exists) {
          debug.handlers('getAvailablePath -- path exists: ' + newPath + ' | ' + exists)
          var id = uuid.v1().split('-')[0] + '-'
          newPath = path.join(containerURI, id + slug)
        }
        debug.handlers('getAvailablePath -- path doesnt exist: ' + newPath)
    ...
    function () {
      debug.handlers('getAvailablePath -- generated path: ' + newPath)
      callback(newPath)
    })
}
