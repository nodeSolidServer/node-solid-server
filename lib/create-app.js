...
function createApp (argv = {}) {
  ...
  if (ldp.webid) {
    ...
    app.post('/api/accounts/signin', bodyParser.urlencoded({ extended: false }), (req, res, next) => {
      if (!validUrl.isUri(req.body.webid)) {
        return res.status(406).send('This is not a valid URI')
      }

      request.options(req.body.webid, function (err, req) {
        if (err) {
          res.status(409).send('Did not find a valid endpoint')
        }
    ...
    app.post('/api/accounts/signout', (req, res, next) => {
      req.session.userId = ''
