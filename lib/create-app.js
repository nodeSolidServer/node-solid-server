...
function startOidc (argv) {
  var oidcRpClient = new OidcRpClient({
    configFolder: argv.oidcClientStore
  })
  const clients = fs.readdirSync(argv.oidcClientStore)

  clients
    .filter(clientName => clientName.endsWith('.json'))
    .map(clientName => clientName.substr(0, clientName.length - 5))
    .forEach((clientId) => {
      Sync(() => oidcRpClient.get(clientId).then.sync(null))
    })

  Sync(() => oidcRpClient.ensureTrustedClient.sync(null, argv.oidc))
  return oidcRpClient
}
...
function createApp (argv = {}) {
  ...
  if (oidcConfig && ldp.auth === 'oidc') {
    ...
    const oidcRpClient = startOidc(argv)
