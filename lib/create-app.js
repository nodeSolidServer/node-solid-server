...
function initWebId (argv, app, ldp) {
  ...
  if (argv.createOwnerPod) {
    ...
    accountManager.accountExists(accountData.username).then(exists => {
      if (!exists) {
        let ownerAccount = accountManager.userAccountFrom(accountData)
        accountManager.createAccountFor(ownerAccount)
          .catch(error => {
          ...
          .then(() => {
            ...
            let ownerWebId = accountManager.accountWebIdFor(accountData.username)
            argv.owner = ownerWebId
            app.locals.owner = ownerWebId
            config.setOwner(argv)
          })
      }
    })
  } else if (argv.owner) {
    config.setOwner(argv)
    app.locals.owner = argv.owner
  }
