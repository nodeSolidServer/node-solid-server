...
import { handlers as debug, authentication as debugAuth } from './debug.mjs'
...
function initWebId (argv, app, ldp) {
  ...
  app.use((req, res, next) => {
    ...
    process.stderr.write(`SESSION-CHECK: ${req.method} ${req.path}, userId: ${userId}, origin: ${origin}\n`)
    console.log(`SESSION-CHECK: ${req.method} ${req.path}, userId: ${userId}, origin: ${origin}`)
    debugAuth(`Session check: ${req.method} ${req.path}, userId: ${userId}, origin: ${origin}`)
    ...
    if (!argv.strictOrigin && !argv.host.allowsSessionFor(userId, origin, trustedOrigins) && !isLogoutRequest(req)) {
      debugAuth(`Rejecting session for ${userId} from ${origin}`)
      // Destroy session data
      delete req.session.userId
      debugAuth(`Deleted session userId for ${userId}`)
      // Ensure this modified session is not saved
      req.session.save = (done) => done()
    }
    if (isLogoutRequest(req)) {
      debugAuth(`Logout request, deleting userId`)
      delete req.session.userId
    }
    next()
  })
...
function initAuthentication (app, argv) {
  ...
  // Update app.locals.authMethod to reflect the actual auth method being used
  app.locals.authMethod = auth
