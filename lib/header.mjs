...
import { metadata as debugMetadata, ACL as debugACL } from './debug.mjs'
...
export async function linksHandler (req, res, next) {
  ...
  if (path.extname(filename) === ldp.suffixMeta) {
    debugMetadata('Trying to access metadata file as regular file.')
...
export async function addPermissions (req, res, next) {
  ...
  debugACL(`Permissions on ${resource} for ${session.userId || '(none)'}: ${userPerms}`)
  debugACL(`Permissions on ${resource} for public: ${publicPerms}`)
