...
class PaymentOracleClient {
  // oracleEndpoint // defaults to http://localhost:8402
  ...
  async hasPaid (agent, resource) {
    ...
    console.log('Call to hasPaid(agent, auth)', agent, resource, this.oracleEndpoint)
    const url = `${this.oracleEndpoint}/?agent=${encodeURIComponent(agent.value)}&resource=${encodeURIComponent(resource.value)}`
...
class ACLChecker {
  ...
  async can (user, mode, method = 'GET', resourceExists = true) {
    console.log('can', user, mode, method, resourceExists)
    ...
    console.log('NSS calling aclCheck.accessDenied', agent)
    //                        function accessDenied(kb, doc, directory, aclDoc, agent, modesRequired, origin, trustedOrigins, originTrustedModes = [], oracle) {
    ...
    let accessDenied = aclCheck.accessDenied(acl.graph, resource, directory, aclFile, agent, modes, agentOrigin, trustedOrigins, originTrustedModes, (agent) => {
      if (!currentPaid) {
        console.log('PAYING WOULD HELP!')
        payingWouldHelp = true
      }
      console.log('RETURNING currentPaid', currentPaid)
      return currentPaid
    })
    ...
    if ((method === 'PUT' || method === 'PATCH' || method === 'COPY') && directory) {
      ...
      if (directory.value === dirname(aclFile.value) + '/' && !resourceExists) {
        const accessDeniedAccessTo = aclCheck.accessDenied(acl.graph, directory, null, aclFile, agent, [ACL('Append')], agentOrigin, trustedOrigins, originTrustedModes, () => {
          const currentPaid = this.hasPaid()
          if (!currentPaid) {
            payingWouldHelp = true
          }
          return currentPaid
    ...
    if (accessDenied && payingWouldHelp) {
      this.messagesCached[cacheKey].push(HTTPError(402, 'Payment Required'))
