...
class ACLChecker {
  ...
  async can (user, mode, method = 'GET', resourceExists = true) {
    ...
    let accessDenied = aclCheck.accessDenied(acl.graph, resource, directory, aclFile, agent, modes, agentOrigin, trustedOrigins, originTrustedModes, this.hasPaid)
    ...
    if ((method === 'PUT' || method === 'PATCH' || method === 'COPY') && directory) {
      ...
      if (directory.value === dirname(aclFile.value) + '/' && !resourceExists) {
        const accessDeniedAccessTo = aclCheck.accessDenied(acl.graph, directory, null, aclFile, agent, [ACL('Append')], agentOrigin, trustedOrigins, originTrustedModes, this.hasPaid)
  ...
  static createFromLDPAndRequest (resource, ldp, req, hasPaid) {
