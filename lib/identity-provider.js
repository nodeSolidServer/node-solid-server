...
IdentityProvider.prototype.putGraph = function (uri, graph) {
  ...
  return function (callback) {
    var content = serialize(graph, uri, 'text/turtle')
...
IdentityProvider.prototype.create = function (options, cert, callback) {
  ...
  if (options.inbox) {
    options.inbox = options.url + options.inbox + '/'
  }
...
function createInboxAcl (options) {
  ...
  addAppendAll(graph, options.inbox + options.suffixAcl)
...
IdentityProvider.prototype.newCert = function (req, res, next) {
  ...
  webid('tls').generate({
    spkac: spkac,
    commonName: options.name + ' created on ' + options.date.toDateString(),
