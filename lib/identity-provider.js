...
function addKey (graph, agent, cert, date) {
  var card = agent.split('#')[0]
  var key = card + '#key-' + date.getTime()
  ...
  graph.add(
    ...
    lit(date.toISOString(), '', sym('http://www.w3.org/2001/XMLSchema#dateTime')))
...
IdentityProvider.prototype.newCert = function (req, res, next) {
  ...
  webid('tls').generate({
    ...
    self.getGraph(options.webid, function (err, card) {
      ...
      addKey(card, options.webid, cert, date)
