...
function handler (err, req, res, next) {
  ...
  if (statusCode === 401) {
    setAuthenticateHeader(authMethod, err, locals, res)
  }
 ...
 * @param authMethod {string}
 * @param err {Error}
 * @param locals {Object} app.locals hashmap
 * @param res {ServerResponse}
 */
function setAuthenticateHeader (authMethod, err, locals, res) {
  switch (authMethod) {
    case 'oidc':
      Auth.oidc.setAuthenticateHeader(err, locals, res)
      break
    case 'tls':
