...
async function patchHandler (req, res, next) {
  ...
  try {
    ...
    try {
      ...
      // return error(403, `Patch: not authorized to do that. '. ${err}`)
      return res.status(403).send('Unauthorized patch')
    }

    const isOverQuota = await overQuota(root, serverUri)
    if (isOverQuota) {
        // return reject(error(413, 'Patch: User has exceeded their storage quota'))
    ...
    */
    ...
    var fileContents
    try {
      fs.readFileSync(resource.path, { encoding: 'utf8' })
    } catch (err) {
      if (err.code === 'ENOENT') {
        fileContents = ''
    ...
    graph.applyPatch(patchObject, graph.sym(url), (err) => {
      if (err) {
        ...
        // return error(409, `The patch could not be applied. ${message}`)
      }
      ...
      fs.writeFileSync(resource.path, serialized, { encoding: 'utf8' }, function (err) {
        if (err) {
...
function writeGraph (graph, resource, root, serverUri) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // before check if we are above quota
