...
import sparqlUpdateParser from './patch/sparql-update-parser.mjs'
import n3PatchParser from './patch/n3-patch-parser.mjs'

const debug = debugModule.handlers
...
async function patchHandler (req, res, next) {
  ...
  try {
    ...
    try {
      ...
      // debug('PATCH -- File does not exist, creating new resource. Error:', err.message)
...
function handler (req, res, next) {
  debug('PATCH -- handler called for:', req.originalUrl || req.url)
