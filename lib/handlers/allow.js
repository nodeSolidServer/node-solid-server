...
function allow (mode) {
  return async function allowHandler (req, res, next) {
    ...
    let graph
    try {
      // do not check for permission. Owner has all rights.
      // TODO check permissions on .meta ???
      graph = await ldp.getGraph(rootUrl + '/.meta')
      const SOLID = $rdf.Namespace('http://www.w3.org/ns/solid/terms#')
      const owner = await graph.any(null, SOLID('account'), $rdf.sym(rootUrl + '/'))
      if (userId === owner.uri) return next()
    } catch (e) {
      debug('Fail to get ' + rootUrl + '/.meta ' + e)
      next(req.authError) // TODO check error type
    }
