...
function allow (mode) {
  return async function allowHandler (req, res, next) {
    ...
    // check owner
    // if (accessDenied) {
    // find owner from /meta
    // const rootUrl = ldp.resourceMapper.resolveUrl(req.hostname)
    // console.log('**** alain ' + rootUrl)
    // console.log('**** alain user ' + userId)
    // const body = await this.fetch(rootUrl + '/meta)
    let graph
    const body = await ldp.readResource(rootUrl + '/meta')
    debug(body)
    try {
      graph = await ldp.getGraph(rootUrl + '/meta') // , 'text/turtle')
      debug(graph)
    } catch (e) { debug('**** alain graph\n' + rootUrl + '/meta' + '\n**** alain') }
    try {
      const SOLID = rdf.Namespace('http://www.w3.org/ns/solid/terms#')
      const ownerSt = graph.statementsMatching(null, SOLID('account'), null)
      debug('**** alain ownerSt')
      debug(ownerSt)
      const owner = ownerSt[0].subject.value
      debug('**** alain ' + owner)
    } catch (e) { debug('**** alain find owner\n' + rootUrl + '/meta' + '\n**** alain') }
    /*
    // and check owner
    // if (user === owner) accessDenied = false
    // }
    */
    if (isAllowed) {
      return next()
