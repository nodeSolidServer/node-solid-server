...
var debug = require('debug')('ldnode:get')
...
function handler (req, res, next) {
  ...
  ldp.get(req.hostname, path, baseUri, includeBody, requestedType, possibleRDFType, function (err, stream, contentType, container) {
    ...
    translate(stream, baseUri, contentType, possibleRDFType, function (err, data) {
      if (err) {
        ...
        return next(error(406, err.message))
      }
  ...
  glob(filename, globOptions, function (err, matches) {
    ...
    async.each(matches, function (match, done) {
      ...
      fs.readFile(match, {encoding: 'utf8'}, function (err, fileData) {
        if (err) {
          debug('error in globHandler' + err)
          return done(null)
        }
        aclAllow(match, req, res, function (allowed) {
          ...
          try {
            ...
            debug('error in globHandler' + parseErr)
          }
          return done(null)
        })
      })
