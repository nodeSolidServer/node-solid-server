...
class LdpOperation {
   ...
   *
   * @param [options.conneg] {Negotiator} Content type negotiator instance
   *
...
class LdpGetOperation extends LdpOperation {
   ...
   */
  static async from (options) {
    const { store, target, headers, conneg } = options
    ...
    if (isContainer) {
      ...
      const indexFile = url.resolve(target, '/index.html')

      if (await store.exists(indexFile) && conneg.mediaType() === 'text/html') {
...
class LdpDeleteOperation extends LdpOperation {
  ...
  async perform ({store}) {
    ...
    if (resource.isContainer) {
      ...
      return store.deleteContainer(resource)
    }
