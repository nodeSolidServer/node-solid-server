...
class LdpOperation {
  ...
  writeCommonHeaders ({res, result = {}}) {
    ...
    if (resource) { // delete operation don't return a result, for example
...
class LdpPutOperation extends LdpOperation {
   ...
   * @returns {Promise<object>} Ldp operation result
   */
  async perform ({store}) {
    const resource = await store.resource(this.target)

    return resource.isContainer
      ? this.putContainer({store, container: resource})
      : this.putResource({store, resource})
  }

  async putResource ({store, resource}) {
    try {
      await store.createResource(resource, this.bodyStream)
    } catch (error) {
      // log error here
      throw error
    }
  ...
  async putContainer ({store, container}) {
    if (container.exists) {
...
class LdpDeleteOperation extends LdpOperation {
  ...
  async perform ({store}) {
    ...
    if (resource.isContainer) {
      ...
      container.resourceNames = await store.loadContentsList(container)
