...
export function initialize (app, argv) {
  ...
  app.use('/', (req, res, next) => {
    ...
    oidc.webIdFromClaims(req.claims)
      .then(webId => {
        console.log('webIdFromClaims result:', webId)
        if (webId) {
          req.session.userId = webId
          console.log('Set req.session.userId to:', webId)
      ...
      .catch(err => {
        console.log('webIdFromClaims error:', err.message)
