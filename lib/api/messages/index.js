...
const debug = require('debug')('api:messages')
    ...
    waterfall([
      ...
      (displayName, cb) => {
        ...
        if (req.body.to.split(':') === 'mailto') {
          ...
          cb(error(500, 'Messaging service not available'))
        }
      }
    ], (err) => {
      if (err) {
        next(error(500, 'Failed to send message'))
        return
      }
...
function getLoggedUserName (req, callback) {
  ...
  ldp.graph(webid.hostname, '/' + webid.pathname, baseUri, function (err, graph) {
    if (err) {
      ...
      callback(error(err.status || 500, 'Failed to find user'))
      return
    }
...
function sendEmail (req, vars, callback) {
  ...
  const webid = url.parse(req.session.userId)
  var send = emailService.templateSender({
