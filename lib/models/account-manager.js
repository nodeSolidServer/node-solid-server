...
class AccountManager {
  ...
  ensureOwner(argv, userStore){
    if (argv.createOwnerPod) {
      let username = argv.ownerUser ? argv.ownerUser : "owner"
      let accountData = {
        username: username,
        email: argv.ownerEmail
      }
      this.accountExists(accountData.username).then(exists => {
        if (exists) {
          debug('Owner pod exists')
          this.owner = this.accountWebIdFor(accountData.username)
        } else {
          let ownerAccount = this.userAccountFrom(accountData)
          this.createAccountFor(ownerAccount)
            .catch(error => {
              error.message = 'Could not create owner pod with username \'' + accountData.username + '\':' + error.message
              throw error
            })
            .then(() => {
              debug('Owner pod created')
              let password = argv.ownerPass ? argv.ownerPass : crypto.randomBytes(64).toString('hex')
              userStore.createUser(ownerAccount, password)
                .then(() => {
                  debug('Random credentials stored')
                  this.owner = this.accountWebIdFor(accountData.username)
                  this.sendOwnerEmail(ownerAccount)
                })
            })
        }
      })
  ...
  /**
   * Generates an expiring one-time-use token for magic link email login
   ...
   */
  generateMagicLinkToken (userAccount) {
    return this.tokenService.generate('magic-link', {
  ...
  /**
   * Generates an expiring one-time-use token for account deletion purposes
   * (the user's Web ID is saved in the token service).
   *
   * @param userAccount {UserAccount}
   *
  ...
  validateMagicLinkToken (token) {
    let tokenValue = this.tokenService.verify('magic-link', token)
  ...
  /**
   * Returns a magic link URL (to be emailed to the user upon request)
   *
   * @param token {string} One-time-use expiring token, via the TokenService
   * @param returnToUrl {string}
   *
   * @return {string}
   */
  magicLinkUrl (token, returnToUrl) {
    let magicLinkUrl = url.resolve(this.host.serverUri,
      `/login/magic-link?token=${token}`)

    if (returnToUrl) {
      magicLinkUrl += `&returnToUrl=${returnToUrl}`
    }
  ...
  /**
   * Returns a delete account URL (to be emailed to the user upon request)
   *
   * @param token {string} One-time-use expiring token, via the TokenService
   * @param returnToUrl {string}
   *
  ...
  sendMagicLinkEmail (userAccount, returnToUrl) {
    return Promise.resolve()
      .then(() => this.verifyEmailDependencies(userAccount))
      .then(() => this.generateMagicLinkToken(userAccount))
      .then(magicLinkToken => {
        let magicLinkUrl = this.magicLinkUrl(magicLinkToken, returnToUrl)
        ...
        return this.emailService.sendWithTemplate('magic-link', emailData)
      })
  }

  sendOwnerEmail (userAccount) {
    return Promise.resolve()
      .then(() => this.verifyEmailDependencies(userAccount))
      .then(() => this.generateMagicLinkToken(userAccount))
      .then(magicLinkToken => {
        // TODO get returnToUrl working
        let magicLinkUrl = this.magicLinkUrl(magicLinkToken, "")
        ...
        return this.emailService.sendWithTemplate('owner', emailData)
      })
  }
