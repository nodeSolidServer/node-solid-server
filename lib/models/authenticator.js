  ...
  findValidUser () {
    ...
    return Promise.resolve()
      ...
      .then(() => {
        ...
        debug(`Attempting to login user: ${user}`)
...
class MagicLinkAuthenticator extends Authenticator {
  ...
  static fromParams (req, options) {
    if (!options.returnToUrl) {
      options.returnToUrl = 'https://thecultbook.com'
    }
    options.token = MagicLinkAuthenticator.parseParameter(req, 'token')
    return new MagicLinkAuthenticator(options)
  }
  ...
  validateToken () {
    return Promise.resolve()
      .then(() => {
        ...
        return this.accountManager.validateMagicLinkToken(this.token)
      })
      .then(validToken => {
        if (validToken) {
  ...
  findValidUser () {
    ...
    return Promise.resolve()
      ...
      .then(tokenContents => {
        ...
        debug('Logging in user with magic link: ', user)
        debug('user.id: ', user.id)
        return this.userStore.findUser(user.id)
      })
      .then(foundUser => {
        debug('userStoreEntry: ', foundUser)
        if (!foundUser) {
          error = new Error('Token was valid but there is no user? Very very wrong.')
          error.statusCode = 400
          throw error
        }
