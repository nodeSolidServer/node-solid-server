...
docker build -t solid-crud https://github.com/solid/test-suite.git#add-testers:/testers/solid-crud
docker build -t solid-crud https://github.com/solid/test-suite.git#add-testers:/testers/web-access-control
...
docker logs server
docker run --rm --name tester --network=testnet --env-file /tmp/env-vars-for-test-image.list webid-provider
