...
describe('HTTP APIs', function () {
      ...
      server.get('/sampleContainer/solid.png')
        ...
        .end(function (err, res) {
          ...
          const size = fs.statSync(path.join(__dirname,
            '../../test/resources/sampleContainer/solid.png')).size
  ...
  describe('PUT API', function () {
    const putRequestBody = fs.readFileSync(path.join(__dirname,
      '../../test/resources/sampleContainer/put1.ttl'), {
      encoding: 'utf8'
    })
    ...
    it('should create new resource and delete old path if different', function (done) {
      server.put('/put-resource-1.ttl')
        ...
        .end(function (err) {
          ...
          if (fs.existsSync(path.join(__dirname, '../../test/resources/put-resource-1.ttl$.txt'))) {
  ...
  describe('POST API', function () {
    ...
    const postRequest2Body = fs.readFileSync(path.join(__dirname,
      '../../test/resources/sampleContainer/post2.ttl'), {
      encoding: 'utf8'
    })
      ...
      server.post('/post-tests/')
        ...
        .end((err, res) => {
          ...
          try {
            ...
            const createdDir = fs.statSync(path.join(__dirname, '../../test/resources', postLocation.slice(0, -1)))
      ...
      server.post('/post-tests/')
        ...
        .end((err, res) => {
          ...
          try {
            ...
            const createdDir = fs.statSync(path.join(__dirname, '../../test/resources', expectedDirName))
            assert(createdDir.isDirectory(), 'Container should have been created')
          } catch (err) {
            return done(err)
          }
    ...
    it('should create as many files as the ones passed in multipart',
      function (done) {
        server.post('/sampleContainer/')
          .attach('timbl', path.join(__dirname, '../../test/resources/timbl.jpg'))
          .attach('nicola', path.join(__dirname, '../../test/resources/nicola.jpg'))
          .expect(200)
          .end(function (err) {
            if (err) return done(err)

            const sizeNicola = fs.statSync(path.join(__dirname,
              '../../test/resources/nicola.jpg')).size
            const sizeTim = fs.statSync(path.join(__dirname, '../../test/resources/timbl.jpg')).size
            const sizeNicolaLocal = fs.statSync(path.join(__dirname,
              '../../test/resources/sampleContainer/nicola.jpg')).size
            const sizeTimLocal = fs.statSync(path.join(__dirname,
              '../../test/resources/sampleContainer/timbl.jpg')).size
